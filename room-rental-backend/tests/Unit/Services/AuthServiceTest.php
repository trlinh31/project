...
class AuthServiceTest extends TestCase
{
    ...
    {
        $password = '123456';
        $user = Admin::factory()->withPassword($password)->unverified()->make();
        $userData = Arr::add(Arr::only($user->getAttributes(), ['username', 'email']), 'password', $password);
    ...
    private function validateRegister($tableName, $user, $result)
    {
        $this->assertDatabaseHas($tableName, $user->toArray());
        $this->assertArraySubset(Arr::except($user->toArray(), 'email_verified_at'), $result['user']->toArray());
    ...
    public function test_admin_revoke_all()
    {
        $loginData = [
            'username' => 'admin',
            'password' => '123456'
